    ...
    def __load_bls_key(self):
        ...
        try:
            ...
            if not os.path.exists(self.config["bls_private_key_store_path"]):
                logger.error(
                    f"BLS key file not found at: {self.config['bls_private_key_store_path']}"
                )
    ...
    def __load_clients(self):
        ...
        try:
            cfg = BuildAllConfig(
                ...
                registry_coordinator_addr=self.config[
    ...
    def __load_task_manager(self):
        ...
        try:
            ...
            if not os.path.exists(task_manager_abi_path):
                logger.error(
                    f"Task manager ABI file not found at: {task_manager_abi_path}"
                )
